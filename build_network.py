...
# External inputs
exc_bg_chn = NetworkBuilder('exc_bg_chn')
exc_bg_chn.add_nodes(N=numAAC,
                     pop_name='tON',
                     potential='exc',
                     model_type='virtual')
...
def dist_conn_perc_angle(src, trg, min_dist=0.0, max_dist=300.0, min_syns=1, max_syns=2, A=0.2, B=0.2):
    ...
    if sid == tid:
        ...
        perp_dist = np.linalg.norm(np.cross((trg_pos - src_pos), (trg_pos - vec_pos))) / np.linalg.norm(
            (vec_pos - src_pos))
...
def perc_conn(source, target, perc=0.2):
    if np.random.random() < perc:
        # print("connecting cell {} to {}".format(sid,tid))
        tmp_nsyn = 1
    else:
        return None
...
conn = net.add_edges(source={'pop_name': 'AAC'}, target={'pop_name': 'PyrA'},
                     iterator='one_to_one',
                     connection_rule=dist_conn_perc,
                     connection_params={'min_dist': 0.0, 'max_dist': 100000.0,
                                        'min_syns': 1, 'max_syns': 2, 'A': 0.313, 'B': 0.004029},
...
conn = net.add_edges(source={'pop_name': 'AAC'}, target={'pop_name': 'PyrC'},
                     iterator='one_to_one',
                     connection_rule=dist_conn_perc,
                     connection_params={'min_dist': 0.0, 'max_dist': 100000.0,
                                        'min_syns': 1, 'max_syns': 2, 'A': 0.313, 'B': 0.004029},
...
net.add_edges(source=thalamus.nodes(), target=net.nodes(pop_name='PyrA'),
              ...
              syn_weight=1.0e-03, # changed from e-04
              weight_function='lognormal',
...
#net.add_edges(source=exc_bg_chn.nodes(), target=net.nodes(pop_name='AAC'),
#              connection_rule=one_to_all,
#              syn_weight=3.0e-04,
#              weight_function='lognormal',
#              weight_sigma=3.0e-04,
#              target_sections=['somatic'],
#              delay=0.1,
#              distance_range=[0.0, 300.0],
#              dynamics_params='AMPA_ExcToExc.json',
